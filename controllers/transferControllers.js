...
const webhook = asyncHandler(async (req, res) => {
    ...
    if (hash === req.headers['x-paystack-signature']) {
        ...
        const notification = new Notification({
            type: 'payment',
            title: `You successfully funded your account with ${currencyFormatter.format(amount / 100, { code: 'NGN' })}`
        })
...
const payHandyman = asyncHandler(async (req, res) => {
    ...
    if (Number(req.body.amount) > Number(user.accountBalance)) {
        ...
        hire.status = 'completed';
        handyMan.accountBalance = Number(handyMan.accountBalance) + Number(handyMan.accountBalance)
