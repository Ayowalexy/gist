...
const addInformation = asyncHandlers(async (req, res) => {
  if (
    Object.keys(req.body).includes("isHandyMan") &&
    typeof req.body.isHandyMan === "boolean"
  ) {
...
const addImages = asyncHandlers(async (req, res) => {
  ...
  await User.findByIdAndUpdate(
    { _id: id },
    {
      $push: { portfolioImages: req.body.images },
    }
  );
  res.status(200).json({
    status: "success",
